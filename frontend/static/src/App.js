...
class App extends Component {
  ...
  componentDidMount() {
    ...
    const startRoom = {
      ...
      static: arch,
      desc:
        "Welcome to BRIDGE, a role playing game engine. To move west to the next screen, click the arrow.",
      west: 25,
      lit: true,
      danger: false,
    };
    ...
    this.timerID = setInterval(() => this.timeHeal(), 10000);
  ...
  meleeAttack(char, mob) {
    ...
    if (charAttack > mobEvade) {
      const damage =
        this.rando(charWeapon.damageLow, charWeapon.damageHigh) +
        char.strBonus +
        char.damageBonus;
      mob.hp = mob.hp - damage;
      this.setState({
        charAttackMessage: `${char.name}'s ${charWeapon.material} ${charWeapon.name} ${charWeapon.damMessage} the ${mob.name} for ${damage} points of damage!`,
      });
    } else {
      this.setState({
        charAttackMessage: `${char.name} misses the ${mob.name}!`,
      });
    }
    if (mob.hp <= 0) {
  ...
  magicAttack(char, mob, charWeapon) {
    ...
    if (charSpell == null) {
      ...
      if (char.sp >= charSpell.spCost) {
        ...
        setTimeout(() => {
          this.setState({
            charAttackMessage: `${char.name}'s ${charSpell.name} ${charSpell.damMessage} into the ${mob.name}, doing ${charDamage} damage!`,
          });
        }, 100);
        ...
        if (char.sp < charSpell.spCost) {
          this.setState({
            playerMessage: `You don't have enough spell points to cast that.`,
          });
          setTimeout(() => {
            this.setState({ playerMessage: "" });
          }, 1200);
  ...
  mobAttack(char, mob) {
    ...
    if (mobAttack > charEvade) {
      ...
      setTimeout(() => {
        this.setState({
          mobAttackMessage: `The ${mob.name} hits ${char.name} for ${mob.damage} points of damage!`,
        });
      }, 1500);
      if (char.hp <= 0) {
        setTimeout(() => {
      ...
      setTimeout(() => {
        this.setState({
          mobAttackMessage: `The ${mob.name} misses ${char.name}!`,
        });
      }, 1000);
    }
    setTimeout(() => {
      this.setState({ charAttackMessage: "" });
    }, 2500);
    setTimeout(() => {
      this.setState({ mobAttackMessage: "" });
    }, 2500);
  }
  ...
  charWins(char, mob) {
    setTimeout(() => {
      this.setState({
        playerMessage: `${char.name} has defeated the ${mob.name}! The fight is over.`,
      });
    }, 1000);
    setTimeout(() => {
      this.setState({
        playerMessage: `${char.name} receives ${mob.xp} points of experience.`,
      });
    }, 2000);
  ...
  awardTreasure(char, mob) {
    ...
    setTimeout(() => {
      this.setState({
        playerMessage: `The ${mob.name} was carrying ${treasure} silver! The money goes into ${char.name}'s stash.`,
      });
    }, 1000);
  ...
  charDeath(char) {
    ...
    this.setState({
      tweetTitle: `${char.name} has been killed! Avenge this fallen hero's death at BRIDGE RPG.`,
    });
    setTimeout(() => {
      this.sendTweet();
    }, 500);
  ...
  startRandomFight() {
    ...
    if (mob.isShopkeeper != true) {
      ...
      setTimeout(() => {
        this.setState({
          playerMessage: `A bloodthirsty ${mob.name} emerges from the shadows! \n Will you fight or flee?`,
        });
      }, 500);
    } else {
      this.startRandomFight();
  ...
  async levelUp(char, level) {
    ...
    setTimeout(() => {
      this.setState({
        playerMessage: `${char.name} raises to level ${level}!`,
      });
    }, 5000);
    ...
    setTimeout(() => {
      this.setState({
        levelUp: true,
        playerMessage: `${char.name} gains power! Hit Points: ${hpmax}. Spell Points: ${spmax}. Attack: ${attack}. Armor Class: ${ac}. Congrats! Don't forget to save ${char.name}.`,
      });
    }, 7000);
    this.setState({
      tweetTitle: `${char.name} has gained level ${level}! Join us at BRIDGE to get in on old-school RPG fun!`,
    });
    setTimeout(() => {
      this.sendTweet();
    }, 500);
  ...
  useTorch() {
    ...
    this.setState({
      lightSpell: true,
      playerMessage: `${char.name} lights a torch.`,
    });
    setTimeout(() => {
      this.checkLight();
    }, 100);
    ...
    setTimeout(() => {
      this.setState({ lightSpell: false });
    }, 300000);
  }
  ...
  light() {
    ...
    this.setState({
      playerMessage: `${char.name} casts Light! A hovering flame appears in the air, illuminating the area.`,
    });
    setTimeout(() => {
      this.checkLight();
    }, 100);
  ...
  render() {
    ...
    const spellChoice = (
      <div className="spell-dropdown" value={this.state.charSpell}>
        ...
        <input
          ...
          type="submit"
          onClick={() => this.magicAttack(char, mob, charWeapon)}
          value="Cast!"
        />
      </div>
    );
    ...
    const switchViewsButton = (
      <button onClick={this.changeToCombatWindow}>Switch View</button>
    );
    const getRandomMob = <button onClick={this.randomMob}>Generate Mob</button>;
    const meleeAttackButton = (
      <button
        onClick={() => {
          this.meleeAttack(char, mob, charWeapon);
        }}
      >
        Melee Attack
      </button>
    );
    const magicAttackButton = char.magicUser ? (
      <button
        onClick={() => {
          this.startMagicAttack(char, mob, charWeapon);
        }}
      >
        Cast Spell
      </button>
    ...
    const startButton = (
      <button id="startButton" className="saveButton" Click={this.gameOn}>
        Start Game
      </button>
    );
    ...
    const immWindow = (
      <div id="immWindow">
        <input
          ...
          name="immWindow"
          value={this.state.immWindow}
          onChange={this.handleImmInput}
        />
        <input
          type="text"
          placeholder="arg"
          ...
          onChange={this.handleImmInput}
        />
        <button type="submit" onClick={() => this[command](arg)}>
          Go
        </button>
      </div>
    );
    ...
    const continueButton = (
      <button className="saveButton" onClick={this.resetNow}>
        Continue
      </button>
    );
        ...
        {this.state.startGame == false ? (
          ...
          <div className="container-fluid">
            <div className="row topRow">
              <div
                className={`col-sm-4 box graphicsWindow`}
                style={{ padding: "0px" }}
              >
                <GraphicsWindow all={this.state} />
              </div>
              <div className="col-sm-1 fireCol">
                {this.state.lightSpell === true ? (
                  <img className="fireGif" src={flame} alt="fire" />
                ) : null}{" "}
              </div>
              <div className={`col-sm-7 box textWindow`}>
                ...
                {this.state.combatwindow == false ? (
                  <Rooms
                    ...
                    changeRoomImage={this.changeRoomImage}
                  />
                ) : (
                  <p className="combatButtons">
                    {meleeAttackButton}
                    {magicAttackButton}
                    {runAwayButton}
                  </p>
              ...
              <div className={`col-md box charWindow`}>
                ...
                <React.Fragment>
                  <Switch>
                    <Route
                      path="/account/"
                      children=<Account gameOn={this.gameOn} all={this.state} />
                    />
                    <Route
                      path="/character/create/"
                      children=<CharCreate
                        all={this.state}
                        conjure={this.conjure}
                    ...
                    <Route
                      path="/character/"
                      children=<Character all={this.state} />
                    />
                    <Route
                      path="/inventory/"
                      children=<Inventory
                        useTorch={this.useTorch}
                    ...
                    <Route
                      path="/build/"
                      children=<Builder goto={this.goto} all={this.state} />
                    />
                    <Route
                      path="/main/"
                      children=<CharWindow heal={this.heal} all={this.state} />
                    />
                  </Switch>
                </React.Fragment>
